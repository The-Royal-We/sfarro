...
void * vfs_threadproc(struct Args args) {
	umask(0);
	fprintf(stderr, "Mounting %s to %s\n", args.argv[args.argc - 1],
			args.vfs_data->rootdir);
	fuse_main(args.argc, args.argv, &vfs_oper, &args.vfs_data);
	return 0;
}

void initialize_vfs(int argc, char *argv[], struct vfs_state *vfs_data) {

	struct Args args;
	args.argc = argc;
	args.argv =malloc(argc);
	args.argv = argv;
	args.vfs_data = vfs_data;

	pthread_t vfs_tid;

	pthread_create(&vfs_tid, NULL, &vfs_threadproc, &args);
